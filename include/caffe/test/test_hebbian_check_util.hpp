...
class HebbianChecker {
 ...
 protected:
  Dtype GetObjAndHebbian(const Layer<Dtype>& layer,
...
void HebbianChecker<Dtype>::CheckHebbianSingle(Layer<Dtype>* layer,
  ...
  GetObjAndHebbian(*layer, top, top_id, top_data_id);
  ...
  for (int blob_id = 0; blob_id < blobs_to_check.size(); ++blob_id) {
    ...
    for (int feat_id = 0; feat_id < current_blob->count(); ++feat_id) {
      ...
      if (!element_wise || (feat_id == top_data_id)) {
        ...
        current_blob->mutable_cpu_data()[feat_id] += stepsize_;
        estimated_hebbian = (positive_objective - negative_objective) /
            stepsize_ / 2.;
      }
...
template <typename Dtype>
Dtype HebbianChecker<Dtype>::GetObjAndHebbian(const Layer<Dtype>& layer,
